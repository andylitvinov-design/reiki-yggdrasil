...
export async function loadProfileCabinetBootstrap({
  ...
  onStep("user-has-id");
  onStep("profile-request-skipped-for-recovery");

  return {
    currentUser,
    currentProfile: null,
    notices: [
      "Профиль и материалы временно загружаются в безопасном базовом режиме. Кабинет открыт без ожидания дополнительных данных."
    ]
  };
}
