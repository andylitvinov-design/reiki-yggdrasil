...
const DAO_FULU_STYLE_VALUES = new Set([
  "fu-paper-slip",
  "cloud-register",
  "thunder-tablet",
  "taofu-charm"
...
export default function ProfileLitePowerPlaceModule({
  ...
  const renderDaoFuluStyle = (daoStyle) => (
    <div className={`daoFuluStyleFrame dao-fulu-style-${daoStyle}`} aria-label="ДАО талисман">
      <div className="daoFuluStyleHeader" aria-hidden="true">
      ...
      <div className="daoFuluSealBox" aria-hidden="true">
        <span className="daoFuluCloudCorner daoFuluCloudCorner--tl" aria-hidden="true" />
        <span className="daoFuluCloudCorner daoFuluCloudCorner--tr" aria-hidden="true" />
        <span className="daoFuluCloudCorner daoFuluCloudCorner--bl" aria-hidden="true" />
        <span className="daoFuluCloudCorner daoFuluCloudCorner--br" aria-hidden="true" />
      </div>
              ...
              <div className={`powerPlacePrintArea field-layout-${compositionDraft.field_layout || "square"}`} style={sourceSlotScaleStyle}>
                <div className={[`powerMandalaPanel field-layout-${compositionDraft.field_layout || "square"} outer-cover-${outerCover?.type === "image" ? "image" : outerCover?.tone || "none"} ${outerCoverClass}`, !visibilitySettings.center ? "power-place-hide-center" : "", !visibilitySettings.slots ? "power-place-hide-slots" : "", !visibilitySettings.outer_cover ? "power-place-hide-outer-cover" : "", !visibilitySettings.inner_cover ? "power-place-hide-inner-cover" : ""].filter(Boolean).join(" ")} style={{ ...(outerCover?.type === "image" ? { "--power-outer-cover-image": `url(${coverDisplaySrc(outerCover)})` } : {}), ...sourceSlotScaleStyle }}>
                  ...
                  {compositionDraft.constructor_type === "client" ? (
                    ...
                    <div className={`daoMandalaSheet${(compositionDraft.__dao_style || "style-1") === "talisman-1" ? " dao-talisman" : (compositionDraft.__dao_style || "style-1") === "talisman-2" ? " dao-talisman-2" : DAO_FULU_STYLE_VALUES.has(compositionDraft.__dao_style || "style-1") ? ` dao-style-${compositionDraft.__dao_style}` : ""} cover-${innerCover?.tone || "gold"} ${innerCoverClass}`.trim()} style={innerCoverImageStyle(innerCover, coverDisplaySrc(innerCover))}>
                      ...
                      ) : DAO_FULU_STYLE_VALUES.has(compositionDraft.__dao_style || "style-1") ? (
                        renderDaoFuluStyle(compositionDraft.__dao_style)
