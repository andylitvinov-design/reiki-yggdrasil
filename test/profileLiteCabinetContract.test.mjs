...
// extractCssUrls must only be called inside openPowerPlacePdfPrintView, never at render or load time
...
// The function openPowerPlacePdfPrintView must exist and call extractCssUrls internally
...
{
  // Remove the function body of openPowerPlacePdfPrintView and the import line, then check no remaining call site
...
// preloadImagesForPrint must exist (not the old preloadImages) with safe 2500ms timeout
...
// requestAnimationFrame fallback must exist in raf2
...
// auth branch (when !user or authStatus !== success) must not render any PDF-related logic
{
