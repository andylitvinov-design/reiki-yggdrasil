...
Status: список собран по пользовательскому скриншоту. Прямые URL отдельных курсов и прямые video/embed URL пока не подтверждены, поэтому UI показывает красивое видео-место-заглушку и кнопку `Открыть источник`. После проверки точных ссылок нужно заменить `courseUrl` / `embedUrl` в `src/data/freeCourseLinks.js`.
...
- видеоокно `iframe` для выбранного материала, когда `embedUrl` или YouTube `courseUrl` подтверждены;
- безопасная заглушка видеоокна, когда прямой видео URL ещё не подтверждён;
- подкатегории: `Все`, `Греция`, `Египет`, `Майя`, `Планеты`, `Защита`;
- список материалов внутри выбранной категории;
- кнопка открытия выбранного источника в новой вкладке.
...
Validation script: `npm run validate:free-courses`.
